...
public class CarController : MonoBehaviour
{
    ...
    public void FixedUpdate()
    {
        foreach (Axle axle in axles)
        {
            if (axle.isSteering)
            {
                axle.leftWheel.collider.steerAngle = inputMovement.x * maxSteeringAngle;
                axle.rightWheel.collider.steerAngle = inputMovement.x * maxSteeringAngle;
            }
            if (axle.isMotor)
            {
                axle.leftWheel.collider.motorTorque = inputMovement.y * maxMotorTorque;
                axle.rightWheel.collider.motorTorque = inputMovement.y * maxMotorTorque;
            }
